    ...
    public class ProductCardViewComponent : ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync(FilterViewModel filter = null, List<int> productIds = null, bool istype = false, string type = "", bool isFavoritesPage = false)
        {
            ...
            var modell = productsQuery
                                  ...
                                  .Where(p => filter.minEngine <= 0 || (filter.minEngine > 0 && p.EnginePower <= filter.minEngine))
                                  .Where(p => filter.maxEngine <= 0 || (filter.maxEngine > 0 && p.EnginePower <= filter.maxEngine))
                                  .Where(p => filter.minPrice <= 0 || (filter.minPrice > 0 && p.Price <= filter.minPrice))
                                  .Where(p => filter.maxPrice <= 0 || (filter.maxPrice > 0 && p.Price <= filter.maxPrice))
